...
function App() {
  ...
  useEffect(() => {
    // Verify session on app start but do not redirect to a login page.
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (contentType && contentType.includes('application/json')) {
            // Read and ignore JSON body to avoid unused-variable TypeScript errors
            await res.json().catch(() => null);
          }
        }
      } catch (e) {
        console.warn('Session check failed, continuing without login.', e);
      } finally {
        // Allow the app to render regardless of session state
        setAuthChecked(true);
      }
    })();
